            ...
            <div class="modal-body">
                <form id="myForm" action="" method="post" class="form-horizontal" enctype="multipart/form-data">
                    <div class="row">
                        ...
                        <div class="col-sm-12">
                            <div class="form-group">
                                <label class="control-label col-sm-4">Page Content</label>
                                <div class="col-sm-8">
...
<script>
    $(function () {
        ...
        $('#btnAdd').click(function () {
            ...
            $('#myForm').attr('action', '<?php echo base_url() ?>admin/addPage');
            $('#btnSave').show();
        });
        ...
        $('#btnSave').click(function () {
            ...
            $('#btnSave').show();
            //validate form
            var page_name = $('input[name=page_name]');
            var page_content = $('textarea[name=page_content]');
            var result = 1;
            if (page_name.val() === '') {
                alert('blank');
                page_name.parent().parent().addClass('has-error');
            } else {
                page_name.parent().parent().removeClass('has-error');
                result += 1;
            }
            if (page_content.val() === '') {
                alert('blank');
                page_content.parent().parent().addClass('has-error');
            } else {
                page_content.parent().parent().removeClass('has-error');
                result += 1;
            }
            if (result === 3) {
                $.ajax({
        ...
        $('#showdata').on('click', '.item-edit', function () {
            ...
            $('#myForm').attr('action', '<?php echo base_url() ?>admin/updatePage');
            var page_name = $('input[name=page_name]');
            var page_content = $('textarea[name=page_content]');
            $.ajax({
                type: 'ajax',
                method: 'get',
                url: '<?php echo base_url() ?>admin/editPage',
                data: {id: id},
                async: false,
                dataType: 'json',
        ...
        $('#showdata').on('click', '.item-view', function () {
            ...
            $('#myModal').modal('show');
            $('#myForm')[0].reset();
            $('#myModal').find('.modal-title').text('View Page');
            $('#myForm').attr('action', '');
            var page_name = $('input[name=page_name]');
            var page_content = $('textarea[name=page_content]');
            $.ajax({
        ...
        $('#showdata').on('click', '.item-delete', function () {
            ...
            $('#btnDelete').unbind().click(function () {
                $.ajax({
                    ...
                    method: 'get',
                    async: false,
                    url: '<?php echo base_url() ?>admin/deletePage',
                    data: {id: id},
                    dataType: 'json',
                    success: function (response) {
                        if (response.success) {
        ...
        function showAllPages() {
            $.ajax({
                type: 'ajax',
                url: '<?php echo base_url(); ?>admin/showAllPages',
                async: false,
                dataType: 'json',
                success: function (data) {
                    var html = '';
